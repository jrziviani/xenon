...
void exception_handler(const fault &r)
{
    using namespace klib;
    ...
    klib::logger::instance().log("fault addr: 0x%x", faddr);
    klib::logger::instance().log("Page is present: %s", (r.error_code & 0x1) ? "yes" : "no");
...
void interrupt_handler(const fault &r)
{
    ...
    switch (r.int_no) {
        ...
        default:
            klib::logger::instance().log("Interrupt %d ignored", r.int_no);
            break;
    }
}
...
{
    if (irq > 15) {
        klib::logger::instance().log("PANIC: Invalid IRQ %d", irq);
        return;
    }
