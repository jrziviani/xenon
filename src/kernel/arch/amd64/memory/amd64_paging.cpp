...
pte_t *amd64_paging::get_page(paddr_t top_dir, vaddr_t vaddr, bool make)
{
    ...
    if (!PRESENT(pml4_table->dirs[pml4]) && make) {
        ...
        klib::memset(tmp, 0, sizeof(pdpt_t));
    ...
    if (!PRESENT(pdpt_table->dirs[pdpt]) && make) {
        ...
        klib::memset(tmp, 0, sizeof(pde_t));
    ...
    if (!PRESENT(pde_table->dirs[pde]) && make) {
        ...
        klib::memset(tmp, 0, sizeof(pte_t));
...
void amd64_paging::unmap(paddr_t top_dir, vaddr_t vaddr)
{
    ...
    if (!PRESENT(pml4_table->dirs[pml4])) {
        klib::logger::instance().log("PANIC: unmap: pml4 not present");
        return;
    }

    pdpt_t *pdpt_table = reinterpret_cast<pdpt_t*>(ADDRESS(pml4_table->dirs[pml4]));
    if (!PRESENT(pdpt_table->dirs[pdpt])) {
        klib::logger::instance().log("PANIC: unmap: pdpt not present");
        return;
    }

    pde_t *pde_table = reinterpret_cast<pde_t*>(ADDRESS(pdpt_table->dirs[pdpt]));
    if (!PRESENT(pde_table->dirs[pde])) {
        klib::logger::instance().log("PANIC: unmap: pde not present");
        return;
    }

    pte_t *entry = reinterpret_cast<pte_t*>(ADDRESS(pde_table->dirs[pde]));
    if (!PRESENT(entry->pages[pte])) {
        klib::logger::instance().log("PANIC: unmap: pte not present");
        return;
    }
...
paddr_t amd64_paging::create_top_page_directory()
{
    ...
    klib::memset(top_dir_virt, 0x0, sizeof(pml4_t));

    pml4_t *pml4_table = ptr_to<pml4_t*>(ADDRESS(get_current_page()));
    if (!PRESENT(pml4_table->dirs[pml4_index])) {
        klib::logger::instance().log("PANIC: create top dir pagetable");
        return nullptr;
    }
